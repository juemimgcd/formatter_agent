    ...
    return ChatPromptTemplate.from_messages(
        [
            (
                ...
                "如果 top_candidates 中出现 source=llm_local_knowledge 或 url 以 llm:// 开头，"
                "表示联网搜索不可用，此时可以基于模型已有知识生成结构化结果，"
                "但必须在 extraction_notes 中写明 local_knowledge_fallback，"
                "url 字段可以沿用该 llm:// 候选 URL。"
                "如果信息不足，请使用保守默认值。"
